...
export default function AuthCard() {
  ...
  async function handleEmailPassword() {
    ...
    try {
      ...
      if (error) {
        toast.error(error.message || "Sign up failed.", { id: t });
        return;
      }

      // 🔑 Secondary login check to detect "already registered"
      ...
      if (!loginError) {
        // If login works immediately, that means the account already exists
        toast.error("This email is already registered. Please log in instead.", { id: t });
        return;
      }

      // Otherwise, this is a new signup (waiting for email confirmation)
